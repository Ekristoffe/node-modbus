const debug = require('debug')('udp-request')
const CommonRequestBody = require('./request/request-body.js')

/** Class representing a Modbus UDP Request */
class ModbusUDPRequest {
  /** Convert a buffer into a new Modbus UDP Request. Returns null if the buffer
   * does not contain enough data.
   * @param {Buffer} buffer
   * @return {ModbusUDPRequest} A new Modbus UDP Request or Null.
   */
  static fromBuffer (buffer) {
    try {
      ...
      return new ModbusUDPRequest(id, protocol, length, unitId, body)
    } catch (e) {
      debug('not enough data to create a udp request', e)
      return null
    }
  }

  /** Creates a new Modbus UDP Request
  ...
  /** Creates a buffer object representing the modbus udp request.
   * @returns {Buffer} */
  createPayload () {
...
module.exports = ModbusUDPRequest
